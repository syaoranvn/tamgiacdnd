...
const BackgroundDetailsCard = ({ 
  ...
  const getFixedSkills = () => {
    ...
    return background.skillProficiencies
      .filter((sp: any) => !sp.any && !sp.choose)
      .map((sp: any) => Object.keys(sp))
      .flat();
  };
              ...
              <thead>
                <tr>
                  {entry.colLabels.map((label: string, i: number) => (
                    <th key={i} className="border border-slate-600 px-2 py-1 text-left text-slate-200 bg-slate-700">
                      {label}
                    </th>
                  ))}
                </tr>
              ...
              {entry.rows.map((row: any[], i: number) => (
                <tr key={i}>
                  {row.map((cell: any, j: number) => (
                    <td key={j} className="border border-slate-600 px-2 py-1 text-slate-300">
                      {typeof cell === "string" ? (
                        <TextWithTooltips text={cell} />
                      ) : (
                        cell
                      )}
                    </td>
  ...
  // Get all entries (no slice limit)
...
export default function Step4Description({
  ...
  return (
    <div>
      <h2 className="mb-4 font-display text-2xl text-ink">Bước 4: Mô tả nhân vật</h2>
      <p className="mb-6 text-slate-600">
        Thêm thông tin cá nhân, xuất thân, và tính cách cho nhân vật của bạn.
      </p>
